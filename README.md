## ansible-playbooks

是时候使用[Ansible](https://ansible.com/)了

### 官方文档速查

* [UserGuide - v2.9](https://docs.ansible.com/ansible/2.9/modules/modules_by_category.html)
    * [Modules - v2.9](https://docs.ansible.com/ansible/2.9/modules/modules_by_category.html)
        * [debug](https://docs.ansible.com/ansible/2.9/modules/debug_module.html#debug-module)
        * [ping](https://docs.ansible.com/ansible/2.9/modules/ping_module.html#ping-module)
        * [copy](https://docs.ansible.com/ansible/2.9/modules/copy_module.html#copy-module)
        * [template](https://docs.ansible.com/ansible/2.9/modules/template_module.html#template-module)
        * [file](https://docs.ansible.com/ansible/2.9/modules/file_module.html#file-module)
        * [lineinfile](https://docs.ansible.com/ansible/2.9/modules/lineinfile_module.html#lineinfile-module)
        * [synchronize](https://docs.ansible.com/ansible/2.9/modules/synchronize_module.html#synchronize-module)
        * [get_url](https://docs.ansible.com/ansible/2.9/modules/get_url_module.html#get-url-module)
        * [unarchive](https://docs.ansible.com/ansible/2.9/modules/unarchive_module.html#unarchive-module)
        * [cron](https://docs.ansible.com/ansible/2.9/modules/cron_module.html#cron-module)
        * [yum](https://docs.ansible.com/ansible/2.9/modules/yum_module.html#yum-module)
        * [yum_repository](https://docs.ansible.com/ansible/2.9/modules/yum_repository_module.html#yum-repository-module)
        * [apt](https://docs.ansible.com/ansible/2.9/modules/apt_module.html#apt-module)
        * [apt_repository](https://docs.ansible.com/ansible/2.9/modules/apt_repository_module.html#apt-repository-module)
        * [service](https://docs.ansible.com/ansible/2.9/modules/service_module.html#service-module)
        * [shell](https://docs.ansible.com/ansible/2.9/modules/get_url_module.html#get-url-module)
        * [hostname](https://docs.ansible.com/ansible/2.9/modules/hostname_module.html#hostname-module)
        * [user](https://docs.ansible.com/ansible/2.9/modules/user_module.html#user-module)
        * [group](https://docs.ansible.com/ansible/2.9/modules/group_module.html#group-module)
    * [Jinjia2 - v2.9](https://docs.ansible.com/ansible/2.9/user_guide/playbooks_templating.html)
    * [Roles - v2.9](https://docs.ansible.com/ansible/latest/user_guide/playbooks_reuse_roles.html)
    * [SpecialVariables - v2.9](https://docs.ansible.com/ansible/latest/reference_appendices/special_variables.html)

##### 其他参考

* [ansible_facts](_ref/all-facts.json)
